...
int main(int argc, char* argv[])
{
	...
	char destinationFolder[BUFF_SZ] = "./Loan/Result/Approved/";
	char buff[BUFF_SZ];
	const char s[2] = "\n";
	const char t[2] = " ";
	char *arguments[4]={"mv",dest, destinationFolder, NULL}; //Moving the file to the Approved folder
	...
	if ((wbytes = write(fd_id, "Awaiting signature", strlen("Awaiting signature"))) == -1) //Update the loan status and if the update fails, return an appropriate message
	{
		perror("write 1\n");
		exit(1);
	}
	...
	switch (id_fork = fork()) 
		{//cases for the fork: for the son, for a problem and for the father.
			case -1:
				...
				exit(1);
			case 0:
				execvp(arguments[0], arguments); //Moving the file to the Approved folder
				printf("ERROR!\n");
				exit(1);
			default:
				break;
